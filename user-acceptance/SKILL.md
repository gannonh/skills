...
## Output Contract
1. Start with `Scope` in 2-5 bullets (what changed, in user terms).
2. Declare `Mode`: `user-facing`, `non-user-facing`, or `mixed`.
3. Validate in small slices with explicit pass/fail capture per slice.
4. End with `Recommendation`: `GO`, `GO with follow-ups`, or `NO-GO`.
