...
Companion to `ps-how` and `ps-why`. `ps-how` tells you what the code does. `ps-why` tells you why it's shaped that way. Blast radius tells you what it breaks somewhere else.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Use `ps-why` step 2 to pull the PR and commits.
...
4. Be honest about each risk. Give it a real chance of happening and a real cost if it does. Keep the risks you confirmed; list the ones you checked and cleared separately. Same rules as `ps-why`. Cite a real `file:line`, a search that finds nothing is still an answer, and never make up a caller or an API.
...
6. For a big or wide change, run it as an `ps-arena`. Ask several models the same question and merge the answers. Different models catch different real bugs.
...
Write it through `ps-unslop`, cite real code, and strip anything private before it goes anywhere public.
