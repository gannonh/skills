...
Launch all reviewers in a single message using the subagent mechanism. Use the `interrogate reviewers` list from `.agents/pstack/models.json` when present, one reviewer per entry, extending or shrinking the Reviewer A/B/C/D labels below to the configured entry count; otherwise use the table defaults.
...
- Assign the model from the configured `interrogate reviewers` entry, or the table default with no configured line
