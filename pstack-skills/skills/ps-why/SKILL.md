...
Companion to the `ps-how` skill. `ps-how` answers what the code does and how it works. `ps-why` answers what forces led to its shape.
...
Before spawning investigators, list the available MCPs from the active harness. Use the available-tools map when present. Otherwise inspect the `mcps/` directory the harness exposes for enabled MCP servers.
...
- Give it the tool access required for the task. (agent mode). The synthesizer's quality check spot-verifies citations, which can require MCP access. Readonly/Ask mode strips MCPs and defeats that.
...
After the Sources Consulted block, if the user's `ps-why` question is a precursor to actually changing this code, convert the lineage findings into a Preserve / Change / Avoid / Risk constraint set suitable for planning the change.
