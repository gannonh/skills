...
Locate the active workspace's transcripts before fanning out. The system prompt names the workspace's `agent-transcripts/` directory. Use only that path. Don't glob across `unrelated global transcript directories/`. That crosses workspace boundaries and reads private chats from unrelated projects.
...
Mining misses intent that hasn't come up yet. Use the asking the user tool (structured multi-choice) rather than asking the user to type from scratch. Lower cognitive load, higher hit rate.
...
The **ps-poteto-mode** skill shows the shape. Read it for granularity. Don't copy its content; the user's rules are not the same as poteto-mode's.

### 4. Draft the skill

Use the harness's built-in `create-skill` skill to author the skill. Placement:
...
Apply the **ps-unslop** skill and `create-skill`'s writing guidelines to every line. Both apply to any agent-read prose, not just skills.
...
- The **ps-poteto-mode** skill: example of the output shape.
- The **ps-unslop** skill: prose discipline for every line.
- the harness's built-in `create-skill` skill: skill authoring process and writing guidelines.
