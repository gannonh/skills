...
**PRs.** Run an installed code-cleanup skill over the diff before commit. Run `ps-no-comments` before review. Write every PR title, PR description, and commit body with `ps-technical-writing`, then apply `ps-unslop`. Apply every technical-writing layer except Diátaxis. Use one word for each action, keep articles, and avoid `-ing` when a plain verb works.

**Titles.** Use Conventional Commits in the form `type(scope): subject`. Use `feat`, `fix`, `docs`, `refactor`, `test`, `chore`, or `perf` as the type. Use the changed area, such as `pstack` or `ps-poteto-mode`, as the scope. Keep the subject short and imperative. Apply the same `ps-technical-writing` and `ps-unslop` pass as the body. Name a real symbol when one carries the change. For example, `fix(pstack): retarget opening-a-pr babysit trigger`. Do not add a trailing period.
...
- `## Verification`. State how you ran each check and its rigor. Name the real path, such as a CLI-driving capability, a UI-driving capability, or the targeted tests. State the outcome of each check, not only the command name.
...
A subagent that opens a PR runs `ps-interrogate`, an installed code-cleanup skill, and `ps-no-comments`. It returns the URL and does not babysit. Return to the parent.
