...
2. `ps-how` to ground hypotheses; don't claim a perf ceiling without running it first.
...
3. Plan the fix from the trace. If it crosses a function boundary, `ps-architect` first. Delegate implementation to a subagent using your configured perf-issue model (default `large`); review the diff. Capture a post-fix trace.
