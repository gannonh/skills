---
name: okf
description: Create, read, audit, visualize, validate, and maintain Open Knowledge Format documentation bundles in repositories. Use this skill when the user asks for /okf, OKF, Open Knowledge Format, docs-as-knowledge, reading project context from docs, converting a documentation repository into an agent-readable bundle, auditing documentation quality, generating an interactive knowledge graph, updating durable documentation after work, or adding AGENTS.md instructions for agents to consume and maintain the bundle. Supports /okf read, /okf audit, /okf init, /okf update, /okf validate, and /okf viz workflows.
---

# OKF

Use this skill to read, audit, initialize, update, validate, or visualize an Open Knowledge Format bundle.

Before editing, read `references/SPEC.md` from this skill directory. Treat it as the source of truth for OKF v0.2. Keep these layers separate:
...
Generate an interactive graph viewer:

```bash
python <skill-dir>/scripts/visualize_okf.py <repo-root>
```

The default bundle root is `./docs`. The default viewer output is `<bundle>/viz.html`.
...
- `/okf viz`: render the bundle as an interactive HTML knowledge graph.
...
- Do not log formatting, typo-only edits, link-only repairs, generated-index refreshes, or viewer regeneration.
...
2. Read the map.
   - Read the root index when present and relevant section indexes.
...
4. Report loaded context.
   - Summarize relevant concepts and constraints.
...
4. Normalize concepts.
   ...
   - Migrate legacy `timestamp` to `generated.at` only when producer identity and meaningful-change time can be represented honestly.
...
6. Update AGENTS.md.
   - Tell agents where the bundle root is and how to read it.
...
1. Determine what changed from diffs, commits, PR context, source changes, or the user's completed work.
2. Read the current index and relevant concepts before editing. Inspect lifecycle and provenance fields and check for canonical, duplicate, deprecated, or conflicting coverage.
...
4. Maintain navigation and useful cross-links while preserving unmarked editorial prose.
5. Add one concise semantic log entry only when the bundle uses logs and the change merits it.
6. Validate and report concepts created, updated, deprecated, indexes refreshed, provenance changed, and remaining gaps.
...
Use this read-only workflow when the user asks to visualize, browse, or inspect the OKF link graph.

1. Locate the bundle.
   - Use the named bundle root, otherwise `./docs`.
   - Do not rewrite concepts, indexes, logs, or frontmatter as part of visualization.
2. Generate the viewer:
   ...
   ```bash
   python <skill-dir>/scripts/visualize_okf.py \
     --bundle path/to/bundle \
     --out /tmp/knowledge.html \
     --name "Knowledge Base"
   ```

3. Report what was generated.
   - Give the output path and concept, edge, byte, and warning counts.
   - Mention that the HTML embeds the bundle data and viewer code, while Cytoscape.js and marked load from jsDelivr when the page opens.
   - Treat skipped malformed concepts as warnings rather than silently omitting them.
   - Do not commit `viz.html` unless the user asks or the repository already versions generated viewers.

The viewer provides a force-directed concept graph, type colors, search, type filtering, selectable layouts, rendered Markdown details, v0.2 status/trust/freshness signals, internal concept navigation, and backlinks. It resolves both bundle-relative absolute links such as `/guides/setup.md` and ordinary relative links.
...
- Read `./docs/index.md` and relevant section indexes before substantial work.
...
`scripts/visualize_okf.py` supports:

- one interactive HTML output file with embedded bundle data and viewer code
- force-directed and alternate Cytoscape.js layouts
- search and concept-type filtering
- rendered Markdown concept details
- v0.2 status, trust-tier, provenance, and freshness display
- graph edges and in-view navigation for absolute and relative Markdown concept links
- computed backlinks and warnings for malformed concepts
...
- visualization output was generated only when requested and was not committed unintentionally
