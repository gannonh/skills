...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `<project-skills-dir>/verify-*/`). Several candidates → ask which one; none → stop and point at `references/verification/create.md` instead of inventing a target.
