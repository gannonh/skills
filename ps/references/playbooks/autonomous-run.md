...
2. Pick the wake mechanism the harness provides (a loop or schedule command, else a polling heartbeat). An event to watch (CI, a merge, a ref advancing) gets a watcher subagent that wakes you on the event, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.
