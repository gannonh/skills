...
2. Reduce the artifact to the smoking gun: the function on the hot path, the retainer chain from the leaked object to a GC root, the loop firing without input. Parse large artifacts in a subagent ([Guard the Context Window](references/principles/guard-the-context-window.md)), keep the reduced finding in the main thread.
