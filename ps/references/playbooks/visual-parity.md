...
4. Verify each component against its baseline via image diff on the matching surface, driven with the `agent-browser`, `chrome-cdp`, or `automating-ios-simulator` skill as the surface demands. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Loop per component until the diff is zero.
