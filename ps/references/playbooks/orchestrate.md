...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code: conflicted merges and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap; queueing finished work behind an idle integrator is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, resumed, and drained per `references/delegation.md`. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out, to conserve context. The CLI never spawns, waits, or wakes anything.
...
- **Worker / verifier.** Spawned per `references/delegation.md`. A worker that runs off this machine cannot read the local store, so its brief inlines what it needs or points at repo paths. Prefer fewer, broader workers; one writer per worktree or branch ([Separate Before Serializing Shared State](references/principles/separate-before-serializing-shared-state.md)). Run a unit's verifier on a different model family from its worker where the harness allows.
...
- `preferences.md` is the standing-orders register: numbered lines, one constraint each (model policy, unit sizing, verification bar, forbidden paths, escalation policy). Paste it verbatim into every spawn and every resume; directives decay across resumes, and each dropped one costs a human turn. When you catch yourself restating an instruction, append the line before you act ([Encode Lessons in Structure](references/principles/encode-lessons-in-structure.md)).
...
VERIFY       exact commands or the surface-driving skill to use, plus known gotchas
...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape; a 4KB scaffold around a two-line edit costs more to write and obey than the edit. Spawns on this machine may reference the standing-orders file by store path; verbatim paste is for off-machine spawns and every resume.

A sub-coordinator brief adds its track boundary and unit list, its spawn budget, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line; plus track status and queue delta).
...
6. **Land.** Landing is continuous, never a terminal phase: integration starts with the first verified unit and runs alongside the remaining waves. On heavy repos the integrator is a standing role from wave one, landing units as they verify; on repos where local git is cheap, the coordinator lands verified units itself per Roles. Landing safety governs.
7. **Close.** Drain the final inbox, reconcile every spawned agent to a terminal row (done, abandoned, zombie-reconciled), confirm the predicate on the real artifact, confirm every landed PR has a verdict for its current head SHA, audit the trail per the show-me-your-work workflow including its cross-model review, encode recurring corrections into `preferences.md` or the brief template. Leave the store intact; it is the postmortem.
...
- Drain in batches at four points: the end of a critical section, a track rollup, a watcher wake (arm it on the harness's wake mechanism, with a long heartbeat fallback), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
- Critical sections you finish first: authoring a brief, a merge, a conflict decision, writing a gate, updating the ledger.
...
- Units branch off trunk and land independently. Only genuinely dependent work serializes, and it does so as merge-then-branch, never as a chain of open PRs.
- Exactly one integrator merges at a time; record the holder in the standing orders. Concurrent merges into a moving trunk produce conflicts nobody owns.
- Workers never rebase another unit's branch. Babysitters follow `references/playbooks/babysit.md`, one per PR; they report conflicts to the owner rather than resolving them.
- PR closes go through the integrator only. Merges are units with briefs like any other.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, and whatever status surface the harness exposes for a running agent. Transcript mtime is not liveness.
...
- A zombie that returns hours late reconciles against the current queue and ledger before anything is accepted; the world moved while it slept. Salvage unique findings through a fresh unit, never a blind merge.
...
- After a harness restart: agents on this machine are dead, work running elsewhere is not. Re-read the standing orders and `units.tsv`, reattach off-machine work by PR and branch rather than agent id, respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write; `orch` replaces a lock whose holder pid is gone.
...
Never reaches the human: queue nudges, rebase mechanics, retries, CI flake triage, review-thread triage, format fixes, scope the brief already forbids (refuse and continue), and "should I keep going". When in doubt, act and log; deferring is the measured failure mode.

Mid-run discoveries fix only what blocks the queue. Everything else parks in follow-ups; at this fan-out a small scope leak multiplies into PRs nobody asked for.

**Reply:** at checkpoints and close: the predicate and the count against it from `units.tsv` and `ledger.tsv`, tracks and what each landed, the open PR list plus SHAs, verdicts summary, what was abandoned and why, gates awaiting the human (the only asks), the store path, and the trail path. Numbers from the tables, not narrative. Include PR links.
