...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent on the `code` role per `references/delegation.md`, escalated; review the diff. Capture a post-fix trace.
   Apply [Sequence Work into Verifiable Units](references/principles/sequence-verifiable-units.md), verifying each attempt before trying the next.
