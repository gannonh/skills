...
4. **Spawn N parallel candidates** on different models per the arena workflow (`references/arena.md`)'s Phase B. Each works in its own sanitized dir; same prompt to each.
5. **Spawn one blinded judge** on a different model family per the arena workflow (`references/arena.md`)'s Phase C. Judge sees outputs by sanitized label and the rubric, never a model name.
6. **Verify the chain from transcripts, not self-report.** Read each candidate's local transcript, resolved per `references/transcripts.md` and subject to its privacy rule. Look at which files each candidate actually opened. Citing a principle is not reading its file, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
