...
This workflow orchestrates three others: an inline mining pass (see step 1), the `skill-creator` skill (authoring), and the unslop workflow (`references/unslop.md`) (prose discipline). It sequences them; it doesn't replace them.

## Flow

### 0. Check for an existing skill

Look recursively for `*-mode/SKILL.md` matching the user's handle, in the project skills directory and the user-level one (`.agents/skills/`, `~/.agents/skills/`, `~/.claude/skills/`, or the harness's equivalent). Mode skills can live in a personal category subdirectory, not only at the top level. If one exists, confirm intent with the harness's ask-user tool (unless they already said "update my skill" or similar):
...
Locate the active workspace's transcripts before fanning out, per `references/transcripts.md` and its privacy rule.
...
Mining misses intent that hasn't come up yet. Use the harness's ask-user tool (structured multi-choice) rather than asking the user to type from scratch. Lower cognitive load, higher hit rate.
...
This skill's own `SKILL.md` shows the shape. Read it for granularity. Don't copy its content; the user's rules are not the same as ps's.

### 4. Draft the skill

Use the `skill-creator` skill to author the skill. Placement:

- Path: preserve an existing mode skill's category. For a new mode, default to `<handle>-mode/SKILL.md` in the project skills directory, or the user-level skills directory when they prefer a personal skill.
...
Apply the unslop workflow (`references/unslop.md`) and `skill-creator`'s writing guidelines to every line. Both apply to any agent-read prose, not just skills.
...
- This skill's own `SKILL.md`: example of the output shape.
- `references/unslop.md`: prose discipline for every line.
