...
Use this reference when the Babysit playbook (`references/playbooks/babysit.md`) handles comments from a review bot: CodeRabbit, Cursor Bugbot, a GitHub agentic review, or any similar automation. The goal is not to ignore them by default. The goal is to stop treating every comment as a required code change.
...
### Usage the bot cannot see

- Confidence: candidate
- Skip when: The bot flags an export, component, helper, or file as unused, and you can point at the real consumer it could not see (a dynamic import, a generated caller, a sibling PR, a downstream repo).
- Do not skip when: The symbol is public API, or the supposed use cannot be verified by running or reading something.
- Example signal: "Exported component is never used" with a human reply naming a consumer outside the diff.
