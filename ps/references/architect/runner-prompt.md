...
- Shared state: if two actors might both write, ask "what happens?" If the answer isn't "nothing," default to per-actor state with a merge at the read boundary, per [Separate Before Serializing Shared State](references/principles/separate-before-serializing-shared-state.md).
...
- Encode invariants in types: hard-to-misuse types > runtime checks > prose comments, per [Encode Lessons in Structure](references/principles/encode-lessons-in-structure.md).
- Validate at boundaries, trust types inside, per [Boundary Discipline](references/principles/boundary-discipline.md). Business logic as pure functions; the shell stays thin.
- Single source of truth per invariant. Derive instead of sync.
- Idempotent state transitions where applicable, per [Make Operations Idempotent](references/principles/make-operations-idempotent.md). Ask what happens if the operation runs twice or crashes halfway.
- Short call chains. If tracing the flow needs more than three files, flatten the hierarchy, per the **laziness-protocol** and [Minimize Reader Load](references/principles/minimize-reader-load.md).
