...
# Transcripts dir, per harness. See references/transcripts.md for the encodings.
# First existing match wins; empty when none is found, which only blanks LAST_CHAT.
dash_slug=$(printf '%s' "$main_wt" | sed 's#/#-#g')
cursor_slug=$(printf '%s' "$main_wt" | sed 's#^/##; s#/#-#g')
transcripts=""
for candidate in \
	"$HOME/.claude/projects/$dash_slug" \
	"$HOME/.cursor/projects/$cursor_slug/agent-transcripts"; do
	if [ -d "$candidate" ]; then
